  ...
  <body>
    <div class="container">
          ...
          <c:forEach items="${categorias}" var="categoria" varStatus="status">
            <c:if test="${status.index % 4 == 0}">
              <a href="/categories/${categoria.id}"
                ><img
                  src="${categoria.categoryImageUrl}"
                  alt="Image ${status.index}"
              /></a>
              <a href="/categories/${categoria.id}"
                ><p class="nombre">${categoria.nombre}</p></a
              >
            </c:if>
          </c:forEach>
        </div>
        ...
        <ul class="pagination">
          ...
          <c:forEach begin="1" end="${totalPages}" var="pageNumber">
            <li class="page-item ${currentPage == pageNumber ? 'active' : ''}">
              <a class="page-link" href="/categories?page=${pageNumber}"
                >${pageNumber}</a
              >
            </li>
          </c:forEach>
      ...
      <c:if test="${loggedUser.vendedor}">
        <a
          href="/categories/new"
          class="d-inline-block new-category btn btn-success"
          >New Category</a
        >
      </c:if>
    </div>
